...
namespace SECURITY.API
{
    public class SwaggerConfig
    {
                ...
                .EnableSwagger(c =>
                    {
                        ...
                        c.IncludeXmlComments(GetXmlCommentsModelPath());

                        //Con esto podemos incluir el token de seguridad cuando la acción lleva el class decorator [Autorization]
                        c.OperationFilter(() => new AuthTokenHeader());
                        c.OperationFilter(() => new AddDefaultValues());
                        c.OperationFilter(() => new AddFormatValues());
        ...
        //    {
        //        //var aut = swaggerDoc.parameters.Where(x => x.Key == "Authorization").FirstOrDefault();

        //        //if (aut.Key != null)
        //        //{
        //        //    swaggerDoc.parameters.Remove(aut);
        //        //}
        ...
        private class AuthTokenHeader : IOperationFilter
        {
            public void Apply(Operation operation, SchemaRegistry schemaRegistry, ApiDescription apiDescription)
            {
        ...
        [AttributeUsage(AttributeTargets.Method, AllowMultiple = true)]
        public class SwaggerFormatValue : Attribute
        {
            public string ParameterName { get; set; }
            public string Format { get; set; }

            public SwaggerFormatValue(string parameterName, string format)
            {
                ParameterName = parameterName;
                Format = format;
            }
        ...
        private class AddDefaultValues : IOperationFilter
        {
            public void Apply(Operation operation, SchemaRegistry schemaRegistry, ApiDescription apiDescription)
            {
                ...
                foreach (var param in operation.parameters)
                {
                    ...
                    if (apiDescription.ActionDescriptor.GetParameters().Count <= 0) continue;

                    try
                    {
                        var actionParam = apiDescription.ActionDescriptor.GetParameters().First(p => p != null && p.ParameterName == param.name);

                        if (actionParam != null)
                        {
                            if (actionParam.ActionDescriptor.GetCustomAttributes<SwaggerDefaultValue>().Count <= 0)
                                continue;
        ...
        private class AddFormatValues : IOperationFilter
        {
            public void Apply(Operation operation, SchemaRegistry schemaRegistry, ApiDescription apiDescription)
            {
                if (operation.parameters == null)
                ...
                foreach (var param in operation.parameters)
                {
                    ...
                    try
                    {
                        ...
                        if (actionParam != null)
                        {
                            if (actionParam.ActionDescriptor.GetCustomAttributes<SwaggerFormatValue>().Count <= 0)
                                continue;

                            var customAttribute = actionParam.ActionDescriptor.GetCustomAttributes<SwaggerFormatValue>().FirstOrDefault(p => p.ParameterName == param.name);
                            if (customAttribute != null)
                            {
                                param.@format = customAttribute.Format;
                            }
                        }
                    }
                    catch (Exception)
        ...
        {
            return String.Format("{0}{1}", ConfigurationManager.AppSettings["api:xml:path"], "SECURITY.API.xml");
        }

        private static string GetXmlCommentsModelPath()
        {
            return String.Format("{0}{1}", ConfigurationManager.AppSettings["api:xml:path"], "SECURITY.MODEL.xml");
        }
    }
}
